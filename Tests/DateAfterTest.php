...
class DateAfterTest extends BaseRuleTest {

    protected $data = array(
        'valid'             => '1/1/13',
        'invalid_before'    => '1/1/11',
        'invalid_equal'     => '1/1/13'
    );

    public function testValid() {
        $this->validator->addRule( 'valid', new Rule\DateAfter( '1/1/12', 'm/d/y' ) );
        $this->assertTrue( $this->validator->validate( $this->data ) );
    }

    public function testInvalidBefore() {
        $this->validator->addRule( 'invalid', new Rule\DateAfter( '1/1/12', 'm/d/y' ) );
        $this->assertFalse( $this->validator->validate( $this->data ) );
    }

    public function testInvalidEqual() {
        $this->validator->addRule( 'invalid', new Rule\DateAfter( '1/1/13', 'm/d/y' ) );
        $this->assertFalse( $this->validator->validate( $this->data ) );
    }

    public function testInvalidTimezone() {
        $this->setExpectedException( 'InvalidArgumentException' );
        $this->validator->addRule( 'invalid', new Rule\DateAfter( '1/1/13', 'm/d/y', 'this isnt a valid timezone' ) );
        $this->assertFalse( $this->validator->validate( $this->data ) );
    }

    public function testEmptyGarbageNullDataDoesntValidate() {
        $rule = new Rule\DateAfter( '1/1/13', 'm/d/y' );
